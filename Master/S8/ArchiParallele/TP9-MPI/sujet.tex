...
machine sur la première ligne, et une machine voisinne sur la
deuxième ligne.
...
Modifier le programme \texttt{hellow} pour faire communiquer les deux processus en
utilisant :
\begin{itemize}
...
réception. Est-ce une manière correcte de mesurer le temps que prend la
communication ?
...
\subsection{Courbe de latence en fonction de la taille du message}


Ajoutez une boucle pour faire progresser la taille du message \verb+N+
de manière géométrique (de facteur 2) jusqu'à $1024*1024$.
Puis modifiez l'affichage de façon a afficher chaque mesure ainsi :

\verb#fprintf(stderr,"latence %d %ld\n", N, TIME_DIFF(t1,t2));#


Récoltez les performances~: \verb+./ping 2> latence+ puis
utilisez le script pour tracer
une courbe en fonction de la taille :  \verb+Rscript tracer-courbe.R latence+ \\


Pour améliorer la qualité de la courbe,  utilisez une boucle effectuant
la mesure de ping-pong 100 fois et ajoutez avant celle-ci une
...
% \end{verbatim}
 ...
 \section{Mesures de performances (suite)}

 Reprendre le code du programme \verb#ping.c#.
\subsection{Isend}
...
\verb+MPI_Isend()+ et par \verb+MPI_Wait+.  Tracez trois jolies
courbes sur un même graphique à l'aide de 3 printf :

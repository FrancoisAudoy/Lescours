...
{
  const int at = get_global_id(0);
  const int c = at / offset;
  const calc_t p = pos[at];
  const calc_t s = speed[at];
  if (p + s > max[c] - radius ||
      p + s < min[c] + radius)
    speed[at] = -speed[at] ;
}
...
{
  float3 me = load3coord(pos + index, offset);

  for(int i = index + 1; i < natoms; ++i){
    float3 other = load3coord(pos + i, offset);
      if(distance(me,other) == 0){

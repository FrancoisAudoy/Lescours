  ...
  if(e != NULL)
	...
	{case true:
	 ...
	 case Ind: /* (tab_dim k of tau) x   int -> (tab_dim k-1 of tau) */
	   {/* a ecrire */
	     if(e->FG->typno.TYPEF != T_err && e->FD->typno.TYPEF != T_err){
	       if(e->FG->typno.DIM > 0)
		 if(e->FD->typno.TYPEF == T_int){
		   type_copy(&tp,e->FG->typno);
		   e->FG->typno.DIM --;
		   return tp;
		 }
		 else		   
		   typ_error("Indice du tableau non-entier",ligne);
	       else
	...
	case NewAr:                                             /* creation tableau */
	  {
	    if(type_eq(e->FG->typno,terr) || type_eq(e->FD->typno,terr)){
	      type_copy(&tp,terr);
	      type_copy(&(e->typno),terr);
	    }
	    else
